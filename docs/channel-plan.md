...
# Recover Discord Channel Orchestration

## Summary

Pod logs show three concrete production failures:

- Outbound queue is stuck: queued rows exist, but status updates say “not found” thousands of times because the store queries by outbound_id with DynamoDB Limit=1 plus a filter, and an older sent row sorts first.
- Classifier state is stuck: pending=True with stale max_wait_until, so the coordinator keeps fetching Discord history every debounce cycle even when there are no new messages.
- MCP config is masked: the shared workspace contains root opencode.json with empty "mcp": {}, while the correct per-run config exists under .if/; OpenCode can prefer the root project config over OPENCODE_CONFIG.

Implement a forward hotfix, not a rollback.

## Key Changes

- Fix outbox storage/draining:
  - Add an internal registry_sk/outbox key to queued messages returned by query_outbox.
  - Update outbound status by exact pk/sk, not by filtered outbound_id lookup.
  - Make query_outbox page past old sent/failed rows and return only due queued rows.
  - Add bounded no-progress protection so the drainer exits quickly instead of looping for the full lock window.
  - Replace fake idempotency with a real DynamoDB transaction or sentinel row keyed by stable idempotency_key.

- Fix classifier state lifecycle:
  - Add a store method to finish a classifier pass under the lock using the processed latest_observed_event_at.
  - Clear pending, dirty, pending_event_count, batch_first_event_at, max_wait_until, and debounce_until when no newer event arrived during the pass.
  - Preserve pending/dirty only if a newer event arrived while classifying.
  - Stop rescheduling when a pass finds no new user messages.

- Fix OpenCode MCP config:
  - In orchestrated runs, never write root session_dir/opencode.json.
  - Use per-run config plus OPENCODE_CONFIG_CONTENT for classifier/task runs so stale root project config cannot mask scoped MCP tools.
  - Update batch classifier to use its run_id config path/content and pass it into run_opencode.
  - Keep legacy non-orchestrated run_if_flow behavior separate.

- Fix adjacent regressions:
  - Remove literal >>> syntax junk from heartbeat/runner.py and agent/reflection/engine.py.
  - Make classifier parsing resilient to obvious kind/action mismatches or fail the decision without falling back to direct legacy Discord sends.
  - Ensure any fallback response also goes through the outbox.

## Live Data Repair

- For channel 1469338255517286511, after deploying the code fix:
  - Mark stale queued row a84b5989-a36c-4e2d-95ba-fe1098f17d6e as failed/stale.
  - Mark stale task_started row 94ba5754-fc55-46d2-bfbc-8357ae828783 as failed/stale.
  - Keep the current task_completed row 9ced07f3-3d41-4a87-9a5d-c7cdc6cfa5f4 queued only if it is still the desired response; otherwise mark it stale too.
  - Clear the stuck classification state to idle only if no newer Discord event has arrived.

## Test Plan

- Unit tests:
  - Outbox with old sent row before queued rows drains queued rows exactly once.
  - Status update by exact outbox key succeeds.
  - Stable idempotency key prevents duplicate outbox rows.
  - No-new-message classifier pass clears pending and does not reschedule.
  - New event during classification remains pending for a later pass.
  - Orchestrated OpenCode run sets OPENCODE_CONFIG_CONTENT and is not masked by root opencode.json.
  - Heartbeat/reflection files compile.

- Pod verification:
  - Deploy to test/live per repo workflow.
  - Confirm logs no longer repeat “Outbound message ... not found for status update.”
  - Confirm history fetches stop when no new messages exist.
  - Confirm a powerlifting task run sees if_health MCP tools from the scoped config.
  - Confirm Discord receives queued output once, with no chunk interleaving.
