...
async def settings_tag_add(args: dict) -> dict:
    """Add a self-tag to the athlete's own profile (approved=true).
    ...
    def _sync():
        ...
        if tag not in existing_tags:
            tags.append({"tag": tag, "approved": True, "proposed_by": existing["pk"]})
            tags = tags[:MAX_TAGS]
