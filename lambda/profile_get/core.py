...
def _get_settings_by_nickname_sync(user_table, nickname: str, viewer_username: Optional[str]) -> Optional[dict]:
    ...
    while True:
        ...
        for item in resp.get("Items") or []:
            ...
            if (settings.get("nickname") or "").lower() == normalized and _can_view(settings, viewer_username):
                return settings
        last_key = resp.get("LastEvaluatedKey")
        if not last_key:
            break
...
async def profile_get(args: dict) -> dict:
    """Build a public profile by nickname.

    Args:
        args: dict with required `nickname` and optional `viewer_username`
              (requesting user for visibility checks).
    """
    ...
    def _sync():
        settings = _get_settings_by_nickname_sync(user_table, nickname, viewer_username)
        if not settings:
            return None
