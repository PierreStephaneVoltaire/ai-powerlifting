...
async def settings_update_age_class(args: dict) -> dict:
    """Update the age class for a user.

    Args:
        args: dict with required `username`, and `age_class` (string; defaults
              to 'open' when invalid/abscent).
    """
    table = _get_table()
    username = args.get("username") or ""
    age_class = args.get("age_class")
    if age_class is not None and age_class is not None and not isinstance(age_class, str):
        raise ValueError("age_class must be a string or null")

    def _sync():
        existing = _get_existing_sync(table, username)
        if not existing:
            raise ValueError("Settings not found")
        age = age_class if isinstance(age_class, str) and age_class in AGE_CATEGORY_VALUES else "open"
        now = datetime.now(timezone.utc).isoformat()
        table.update_item(
            Key={"pk": existing["pk"]},
            UpdateExpression="SET age_class = :age, updated_at = :now",
            ConditionExpression="attribute_exists(pk)",
            ExpressionAttributeValues={":age": age, ":now": now},
        )
