...
def _get_table():
    ...
    if _table is None:
        ...
        logger.info("[WeightLogAdd] DynamoDB table initialised: %s", table_name)
    return _table
...
async def weight_log_add(args: dict) -> dict:
    """Add (or replace by date) a weight entry in the weight log.

    Args:
        args: dict with optional `pk`, `version` (defaults to "current"),
              and a `date` (YYYY-MM-DD) plus any additional weight fields.
              The full entry shape is forwarded as-is into the entries list.
    """
    table = _get_table()
    pk = _resolve_pk(args)
