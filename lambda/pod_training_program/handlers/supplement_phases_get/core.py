...
def _get_table():
    ...
    if _table is None:
        ...
        logger.info("[SupplementPhasesGet] DynamoDB table initialised: %s", table_name)
    return _table
...
async def supplement_phases_get(args: dict) -> dict:
    """Get the supplement phases array for a program version.
    ...
    def _sync():
        ...
        phases = item.get("supplement_phases") or []
        if not isinstance(phases, list):
            phases = []
        return {"version": version, "sk": sk, "phases": phases}
