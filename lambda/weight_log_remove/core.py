...
def _get_table():
    ...
    if _table is None:
        ...
        logger.info("[WeightLogRemove] DynamoDB table initialised: %s", table_name)
    return _table
...
async def weight_log_remove(args: dict) -> dict:
    """Remove a weight entry by date from the weight log.

    Args:
        args: dict with optional `pk`, `version` (defaults to "current"),
              and required `date` (YYYY-MM-DD).
    """
    table = _get_table()
    pk = _resolve_pk(args)
    ...
    def _sync():
        ...
        log["entries"] = [e for e in entries if e.get("date") != date]
        log["version"] = version
