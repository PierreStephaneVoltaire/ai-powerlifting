...
import { Paper, Group, Text, Badge, Loader, Box, Table, Stack, SimpleGrid, Button } from '@mantine/core';
...
export function AiAnalysis({ effectiveWeeks, weeksMode }: AiAnalysisProps) {
  ...
  const loadCorrelation = useCallback((refresh = false, cacheOnly = true) => {
    if (effectiveWeeks < 4) {
      setCorrReport(null);
      setCorrError(null);
    ...
    setCorrLoading(true);
    setCorrError(null);
    fetchCorrelationReport(effectiveWeeks, 'current', refresh, cacheOnly)
      .then((report) => {
        if (cancelled) return;
  ...
  // Load only the cached AI report automatically. New AI output is generated by the section button.
  useEffect(() => loadCorrelation(false, true), [loadCorrelation]);

  const loadProgramEvaluation = useCallback((refresh = false, cacheOnly = true) => {
    if (weeksMode !== 'block') {
      setEvalReport(null);
      setEvalError(null);
      return;
    ...
    setEvalLoading(true);
    setEvalError(null);
    fetchProgramEvaluation(refresh, cacheOnly)
      .then(setEvalReport)
      .catch((e) => setEvalError(e.message))
      .finally(() => setEvalLoading(false));
  }, [weeksMode, completedCount]);

  // Load only the cached AI report automatically. New AI output is generated by the section button.
  useEffect(() => loadProgramEvaluation(false, true), [loadProgramEvaluation]);
    ...
    <>
      <Paper withBorder p="md">
          ...
          <Group gap="xs">
            ...
            {corrReport && (
              <Badge color={corrReport.cached ? 'blue' : 'green'} variant="light" size="sm">
                {corrReport.cached ? `Cached ${corrReport.generated_at ? new Date(corrReport.generated_at).toLocaleDateString() : ''}` : corrReport.cache_miss ? 'Not generated' : 'Just generated'}
              </Badge>
            )}
          </Group>
          {effectiveWeeks >= 4 && (
        ...
        {effectiveWeeks < 4 ? (
          ...
          <Group gap="xs" py="md">
            <Loader size="xs" />
            <Text size="sm" c="dimmed">Loading ROI analysis…</Text>
          </Group>
        ) : corrError ? (
      ...
      {weeksMode === 'block' && (
        <Paper withBorder p="md">
            ...
            <Group gap="xs">
              ...
              {evalReport && (
                <Badge color={evalReport.cached ? 'blue' : 'green'} variant="light" size="sm">
                  {evalReport.cached ? `Cached ${evalReport.generated_at ? new Date(evalReport.generated_at).toLocaleDateString() : ''}` : evalReport.cache_miss ? 'Not generated' : 'Just generated'}
                </Badge>
              )}
              {evalReport?.stance && !evalReport.cache_miss && (
          ...
          {completedCount < 4 ? (
            ...
            <Group gap="xs" py="md">
              <Loader size="xs" />
              <Text size="sm" c="dimmed">Loading program evaluation…</Text>
            </Group>
          ) : evalError ? (
