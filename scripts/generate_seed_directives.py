...
def generate_put_calls(item):
    ...
    is_global  = bool(dval(item.get("global_directive", {"BOOL": False})))
    superseded = dval(item["superseded_at"]) if "superseded_at" in item else None
    dtype_args = " ".join(dtype_list)
    global_arg = " --global" if is_global else ""
    lines = []
    lines.append("C='{}'".format(single_quote(content)))
    lines.append('put {} {} "{}" "$C" {}{}'.format(alpha, beta, label, dtype_args, global_arg))
    if active is False:
...
def bash_header(count):
    ...
    L.append("  shift 4")
    L.append("  local is_global=false")
    L.append("  local types=()")
    L.append('  for arg in "$@"; do')
    L.append('    if [[ "$arg" == "--global" ]]; then')
    L.append("      is_global=true")
    L.append("    else")
    L.append('      types+=("$arg")')
    L.append("    fi")
    L.append("  done")
    ...
    L.append('      --argjson is_global "$is_global" \\')
    L.append("      '{")
    ...
    L.append("        global_directive:  {BOOL: $is_global},")
    ...
    L.append("")
    L.append('  local global_tag=""')
    L.append('  [[ "$is_global" == "true" ]] && global_tag=" [global]"')
    L.append('  echo "  PUT operator ${sk} ${label} [${types[*]}]${global_tag}"')
    L.append("}")
    L.append("")
    L.append("# ---------------------------------------------------------------------------")
