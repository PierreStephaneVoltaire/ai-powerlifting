...
# ---------------------------------------------------------------------------
# put ALPHA BETA LABEL CONTENT DTYPE...
#   Writes one directive. DTYPE = one or more space-separated type tokens.
# ---------------------------------------------------------------------------
put() {
...
# ---------------------------------------------------------------------------
# mark_inactive ALPHA BETA  --  sets active=false after the put
# ---------------------------------------------------------------------------
mark_inactive() {
  ...
  aws dynamodb update-item \
    --region "$REGION" \
    --table-name "$TABLE" \
    --key '{"pk":{"S":"operator"},"sk":{"S":"'$sk'"}}' \
    --update-expression 'SET active = :v' \
    --expression-attribute-values '{":v":{"BOOL":false}}' \
    --no-cli-pager --output json > /dev/null
  echo "  INACTIVE operator ${sk}"
}

# ---------------------------------------------------------------------------
# mark_superseded ALPHA BETA TIMESTAMP
# ---------------------------------------------------------------------------
mark_superseded() {
  local alpha=$1 beta=$2 ts=$3
  local sk
  sk=$(printf "%02d#%02d#v001" "$alpha" "$beta")
  aws dynamodb update-item \
    --region "$REGION" \
    --table-name "$TABLE" \
    --key '{"pk":{"S":"operator"},"sk":{"S":"'$sk'"}}' \
    --update-expression 'SET superseded_at = :v' \
    --expression-attribute-values "{":v":{"S":"$ts"}}" \
    --no-cli-pager --output json > /dev/null
  echo "  SUPERSEDED operator ${sk} @ ${ts}"
}
...
put 1 25 "IF_REPO_BOOTSTRAP" "$C" architecture code self_aware
...
C='The operator'"'"'s discord name is SirSimpalot. Common nicknames are steph, pierre, sir, simpalot, simp. The Operator prefers to be called Operator'
put 1 29 "OPERATOR_NICKNAMES" "$C" core
...
what it is.'
put 2 42 "PONDERING_PROTOCOL" "$C" memory metacognition
...
put 3 11 "FORMAT_MATCHING" "$C" core personality
...
put 4 6 "TOOL_COMPOSITION" "$C" core tool
...
put 4 7 "OPERATOR_TIMEZONE" "$C" core tool
